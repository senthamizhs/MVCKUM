            ...
            var s = $(r),
                ...
                f = {
                    ...
                    bindUnbindEvents: function (bind) {
                        ...
                        if (n.dragDrop && n.dragDrop.container.length) { //f.isUploadMode() && .... Removed by saranraj for drag & drop issue...
                    ...
                    thumbnails: {
                        ...
                        popup: function (item) {
                            if (!n.thumbnails.popup || !n.thumbnails._selectors.popup) return;
                            ...
                            var template = item.popup.html || $(f._assets.textParse(n.thumbnails.popup.template, item)),
                                ...
                                windowResizeEvent = function () {
                                    ...
                                    if ($childEl && height > $childEl.outerHeight()) height = $childEl.outerHeight();
                                    $node.css({
                                        height: height
                                    })
                                },
                            ...
                            if (item.reader.node) {
                                if (popupIsNew) template.find('.fileuploader-popup-preview .node').html(item.reader.node);
                    ...
                    files: {
                        ...
                        remove: function (item, isFromCheck) {
                            ...
                            if (item.popup && item.popup.html) item.popup.html.remove();
                            if (item.reader.src) {
        ...
        thumbnails: {
            ...
            popup: {
                template: function (data) {
                    return '<div class="fileuploader-popup">' + '<div class="fileuploader-popup-preview">' + '<div class="node ${format}">${reader.node}</div>' + '<div class="tools">' + '<ul>' + '<li>' + '<span>${captions.name}:</span>' + '<h5>${name}</h5>' + '</li>' + '<li>' + '<span>${captions.type}:</span>' + '<h5>${extension.toUpperCase()}</h5>' + '</li>' + '<li>' + '<span>${captions.size}:</span>' + '<h5>${size2}</h5>' + '</li>' + (data.reader && data.reader.width ? '<li>' + '<span>${captions.dimensions}:</span>' + '<h5>${reader.width}x${reader.height}px</h5>' + '</li>' : '') + (data.reader && data.reader.duration ? '<li>' + '<span>${captions.duration}:</span>' + '<h5>${reader.duration2}</h5>' + '</li>' : '') + '<li class="separator"></li>' + (data.format == 'image' && data.reader.src && data.editor ? '<li>' + '<a data-action="crop">' + '<i></i>' + '<span>${captions.crop}</span>' + '</a>' + '</li>' + '<li>' + '<a data-action="rotate-cw">' + '<i></i>' + '<span>${captions.rotate}</span>' + '</a>' + '</li>' : '') + '<li>' + '<a data-action="remove">' + '<i></i>' + '<span>${captions.remove}</span>' + '</a>' + '</li>' + '</ul>' + '<div class="buttons">' + '<a class="fileuploader-popup-button" data-action="cancel">${captions.cancel}</a>' + '<a class="fileuploader-popup-button button-success" data-action="save">${captions.confirm}</a>' + '</div>' + '</div>' + '</div>' + '</div>'
                },
...
$(document).ready(function () {
    //if ($('.i' + 'nnostudi' + 'o').length == 3) {
    window[atob('c2NyZWVuWg==')] = 25  //screenZ
    //}
    $('.expand-1').on('click', function () {
        $(this).closest('p').next('ul').toggleClass('hidden')
    });
